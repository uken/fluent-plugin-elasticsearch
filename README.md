...
Using an _hash generated by the fluentd filter elasticsearch genid plugin and fluentd elasticsearch plugin which is specified id_key as _hash in configuration will communicate to Elasticsearch the uniqueness of the requests so that duplicates will be rejected or simply replace the existing records.
