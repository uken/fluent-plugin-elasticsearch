...
#
module Fluent
  class MultiElasticSearchOutput < ObjectBufferedOutput
    Plugin.register_output('multielasticsearch', self)
  ...
  def format(tag, time, record)
    ...
    def connect(node)
      # I wonder if RestClient Would be a better Option
      Net::HTTP.new(@host, @port.to_i)
    end
    ...
    def on_timer
      ...
      @nodes.each {|n|
        ...
        begin
          $log.trace "sending heartbeat #{n.host}:#{n.port} on TCP"
